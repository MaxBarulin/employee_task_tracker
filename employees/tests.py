...
class TestEmployeeAPI(APITestCase):
    ...
    def setUp(self):
        ...
        # Создаем сотрудников
        self.ivanov = Employee.objects.create(full_name="Иванов", position="Разработчик")
        self.petrov = Employee.objects.create(full_name="Петров", position="Тестировщик")
        ...
        Task.objects.create(name="Task 1 for Petrov", assignee=self.petrov, status=TaskStatus.IN_PROGRESS,
                            deadline="2025-10-10")
        Task.objects.create(name="Task 2 for Petrov", assignee=self.petrov, status=TaskStatus.IN_PROGRESS,
                            deadline="2025-10-11")
        # У Сидорова - 1 активная задача
        Task.objects.create(name="Task 1 for Sidorov", assignee=self.sidorov, status=TaskStatus.IN_PROGRESS,
                            deadline="2025-10-12")
        # У Иванова - 0 активных задач (но одна выполненная, чтобы проверить, что она не считается)
        Task.objects.create(name="Done task for Ivanov", assignee=self.ivanov, status=TaskStatus.DONE,
                            deadline="2025-10-13")
    ...
    def test_busy_employees_endpoint_order(self):
        ...
        # Самое главное: проверяем порядок. 
        # Первым должен быть Петров (2 задачи), последним - Иванов (0 задач).
        # response.data - это список словарей
